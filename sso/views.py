...
def index(request):
    ...
    if 'sso' in req['get_data']:
        ...
        print(errors)
        if not errors:
            print('inside acs with no errors')
            if 'AuthNRequestID' in request.session:
                del request.session['AuthNRequestID']
            request.session['samlUserdata'] = auth.get_attributes()
            print('user attr received is:')
            print(auth.get_attributes())
            print('session value is: ')
            ...
            req['post_data']['RelayState'] = 'http://127.0.0.1:8000/'
    ...
    if 'samlUserdata' in request.session:
        print('SamlUserData is present')
        print(request.session['samlUserdata'])
        paint_logout = True
        if len(request.session['samlUserdata']) > 0:
            attributes = request.session['samlUserdata'].items()
        return render(request, 'app/home.html', {'paint_logout': paint_logout})
